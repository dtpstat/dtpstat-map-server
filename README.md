...
- настраиваемый рейтинг городов: колонки, расчётные метрики, агрегаты, группировки, арифметика, явные приоритеты операций и направление рейтинга;
...
Любое следующее изменение схемы должно быть новой миграцией V015+. Добавление приоритетов арифметических операций схему БД не меняет: они хранятся внутри JSONB `REPORT_CONFIG.METRICS`, поэтому отдельная V015 для этой доработки не требуется.
...
Например, чтобы получить `(A + B) × C`, операции `+` задаётся более высокий уровень, чем `×`. Для `A + (B × C)` — наоборот. Старые настройки без `priority` автоматически получают уровень `1`, поэтому сохраняют прежнее вычисление строго слева направо.
...
Для проекта трамвайных систем можно без изменения кода собрать, например:

```text
№ | город | доля обособления | длина сети (км) | население (тыс.чел)
```

где доля обособления — отношение `SUM(length)` выбранного бизнес-типа `Обособленные` к `SUM(length)` всей сети.
