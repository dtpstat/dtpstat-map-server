...
- публичная карта Mapbox с viewport-загрузкой линий; сервер выбирает полные геометрии по окну с 20% запасом на каждую сторону, чтобы линии не исчезали на границе экрана;
...
Клиент отправляет серверу фактический bbox видимого окна. После обычной API-валидации сервер расширяет selector на **20% ширины и высоты с каждой стороны**, ограничивая координаты WGS84 диапазоном `[-180..180] / [-90..90]`.
