...
test('final V039 geometry model supports durable detached rows and exact effective ownership', async () => {
  const source = await fs.readFile(
    path.join(root, 'db/migrations/V039__effective_geometry_ownership.sql'),
    'utf8',
  );
...
test('V040 replaces stale-row deferred guards with final-state validation', async () => {
  const source = await fs.readFile(
    path.join(root, 'db/migrations/V040__geometry_final_state_constraints.sql'),
    'utf8',
  );
