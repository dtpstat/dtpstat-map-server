...
test('V039 preserves detached legacy geometries instead of requiring CITY_ID', async () => {
  const source = await fs.readFile(
    path.join(root, 'db/migrations/V039__geometry_model_invariants.sql'),
    'utf8',
  );
...
test('schema CI contains a V038-to-current detached geometry upgrade scenario', async () => {
  const source = await fs.readFile(
    path.join(root, 'scripts/verify-geometry-upgrade-ci.js'),
    'utf8',
  );

  assert.match(source, /migration\.version <= 38/);
