...
export function createLineImportRepository() {
  return {
    ...
    insertRawBatch(client, batch) {
      return client.query(INSERT_STREAM_RAW_SQL, [JSON.stringify(batch)]);
    },

    readRawBatch(client, lastSeq, limit) {
      return client.query(
    ...
    createNormalizedStage(client) {
      return client.query(CREATE_STREAM_STAGE_SQL);
    },

    insertNormalizedBatch(client, batch) {
      return client.query(INSERT_STREAM_STAGE_SQL, [JSON.stringify(batch)]);
    },
    ...
    findUnknownBoundaries(client, serializedGeometries = null) {
      return serializedGeometries === null
        ? client.query(FIND_STREAM_UNKNOWN_BOUNDARIES_SQL)
        : client.query(FIND_UNKNOWN_BOUNDARIES_SQL, [serializedGeometries]);
    },

    findBoundaryCityConflicts(client, serializedGeometries = null) {
      return serializedGeometries === null
        ? client.query(FIND_STREAM_BOUNDARY_CITY_CONFLICTS_SQL)
        : client.query(FIND_BOUNDARY_CITY_CONFLICTS_SQL, [serializedGeometries]);
    },

    linkBoundaries(client, serializedGeometries = null) {
      return serializedGeometries === null
        ? client.query(LINK_STREAM_BOUNDARIES_SQL)
    ...
    async applyLineTypeDictionary(client, lineTypes) {
      if (lineTypes.length === 0) return;
      const dictionary = JSON.stringify(lineTypes);
    ...
    findUnknownLineTypes(client, serializedGeometries = null) {
      return serializedGeometries === null
        ? client.query(FIND_STREAM_UNKNOWN_LINE_TYPES_SQL)
        : client.query(FIND_UNKNOWN_LINE_TYPES_SQL, [serializedGeometries]);
    },

    insertGeometries(client, serializedGeometries = null) {
      return serializedGeometries === null
        ? client.query(INSERT_STREAM_GEOMETRIES_SQL)
        : client.query(INSERT_GEOMETRIES_SQL, [serializedGeometries]);
    },
  };
}
