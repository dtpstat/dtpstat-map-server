 ...
 * }} dependencies
 */
export function createLineImportService(pool, dependencies) {
  const repository =
  ...
  if (typeof acquireLock !== 'function') {
    throw new TypeError('Line import acquireLock dependency is required');
  }
  if (typeof recalculateStatistics !== 'function') {
    throw new TypeError(
      'Line import recalculateStatistics dependency is required',
    );
  }
  ...
  return {
    async replaceFromGeoJsonStream(source, operation = {}) {
      ...
      try {
        ...
        const statisticsResult = await recalculateStatistics(client);
