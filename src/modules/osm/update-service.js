...
} from './update-checkpoint-session.js';
import { OsmCityGeometryError } from './update-errors.js';
import { commitOsmBoundaryUpdate } from './update-commit-session.js';
import { processOsmGeometryBatches } from './update-geometry-session.js';
import { loadOsmUpdateIndex } from './update-index-session.js';
import { reportOsmUpdateProgress } from './update-progress-reporter.js';
import {
  buildOsmUpdateResult,
  buildOsmUpdateRunValues,
  finalizeOsmUpdateResult,
} from './update-result.js';
import { createOsmRequestMetricsState } from './update-request-metrics.js';
import { resolveOsmUpdateRuntimeOptions } from './update-runtime-options.js';
...
export function createOsmCityUpdateService(pool, config, dependencies = {}) {
  ...
  const rebuildBoundaryHierarchy = dependencies.rebuildHierarchy;
      ...
      try {
        ...
        const commitResult = await commitOsmBoundaryUpdate({
          ...
          rebuildHierarchy(boundaryClient, { onProgress }) {
            return rebuildBoundaryHierarchy(boundaryClient, {
              signal: operation.signal,
              onProgress,
            });
          },
