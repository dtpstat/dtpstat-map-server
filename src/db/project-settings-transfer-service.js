...
function validateEnvelope(payload) {
  ...
  if (metadata.kind !== SETTINGS_TRANSFER_KIND) {
    throw new ProjectSettingsTransferValidationError(
      `_dtpstat.kind must be ${SETTINGS_TRANSFER_KIND}`,
    );
  }
  if (metadata.schemaVersion !== SETTINGS_TRANSFER_SCHEMA_VERSION) {
    throw new ProjectSettingsTransferValidationError(
      `_dtpstat.schemaVersion must be ${SETTINGS_TRANSFER_SCHEMA_VERSION}`,
    );
  }
  return input;
}
...
const INSERT_LINE_TYPES_STAGE_SQL = `
  INSERT INTO project_settings_line_types_stage (
    name,
    title,
    color,
    line_style,
...
async function rollbackQuietly(client) {
  try {
    ...
    // Preserve the original database/connection failure.
  }
}
...
export function createProjectSettingsTransferService(pool) {
  return {
    ...
    async importSettings(payload) {
      const input = validateEnvelope(payload);
      ...
      const securitySettings = normalizeAdminSecuritySettings(input.securitySettings);
